...
export function buildFrmTsm005Submission(input: ChecklistPayload):
  | {
      ok: true
...
export async function submitFrmTsm005Checklist(input: {
  reportCode: string
  payload: ChecklistPayload
}) {
  "use server"
  ...
  if (input.payload.formCode !== "FRM_TSM_005_REPAIR") {
    return { ok: false, errors: ["Kode form tidak sesuai."] }
  }

  const submission = buildFrmTsm005Submission(input.payload)
  if (!submission.ok) {
    return submission
  }
