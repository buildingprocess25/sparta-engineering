...
export function buildFrmTsm002Submission(input: ChecklistPayload):
  | {
      ok: true
  ...
  if (input.formCode !== "FRM_TSM_002_REPAIR") {
...
export async function submitFrmTsm002Checklist(input: {
  reportCode: string
  payload: ChecklistPayload
}) {
  "use server"
  ...
  if (input.payload.formCode !== "FRM_TSM_002_REPAIR") {
    return { ok: false as const, errors: ["Kode form tidak sesuai."] }
  }

  const submission = buildFrmTsm002Submission(input.payload)
  if (!submission.ok) {
    return submission
  }
